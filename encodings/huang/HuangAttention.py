...
class HuangAttention(nn.Module):
    def __init__(self, d_in, d_out, context_length, dropout, num_heads, max_relative_position=16, qkv_bias=False, auto_regression=False):
        ...
        self.relative_parameters = nn.Parameter(
    ...
    def forward(self, x):
        ...
        relative_matrix = self._generate_relative_positions_matrix(num_tokens) # + num_tokens
        a = self.relative_parameters[relative_matrix + self.clip]#.unsqueeze(-1)

        # Compute scaled dot-product attention (aka self-attention) with a causal mask
        attn_scores = queries @ keys.transpose(2, 3) \
            + torch.einsum('bhid,ijd->bhij', queries, a) \
            + torch.einsum('bhjd,ijd->bhij', keys, a)
